...
- **It is the only non-single-concern module.** The README states the infrastructure is "organized as small, per-concern Terraform modules." Every sibling is a crisp noun—`uploader`, `bucket`/`ingestion`, `queue`, `table`, `extractor`, `alarms`. `results` is the lone three-in-one.
...
The Glue database (`…_results`) and Athena workgroup (`…-results`) keep their names: they name the *result dataset analysts query*, which is accurate, and the overload this ADR resolves is at the Terraform module layer, not the data layer. Renaming them would force catalog-object recreation for no analyst-visible benefit.
...
The root outputs re-point from `module.results.*` to `module.analytics.*` / `module.publisher.*`. The output *keys* already read `analytics_bucket_*` and `publisher_*`; the two that still say `results_` (`results_glue_database_name`, `results_athena_workgroup_name`) are left as-is because they name the result dataset, consistent with the Glue/workgroup naming decision above.

### Migration (no state surgery pre-prod)

Because the module is not deployed to any protected environment—there is no prod, and staging is `force_destroy`-enabled—the split is a straight refactor. The resources are recreated under their new module addresses (`module.results.*` → `module.analytics.*` / `module.publisher.*`); no `moved {}` blocks or `terraform state mv` are required, and a destroy/recreate of the (empty) buckets and catalog objects is acceptable.
...
- The module names match the names the code, outputs, and docs already use; "results" now unambiguously means the DynamoDB table.
...
- No behavioral change. Same Lambda code, same event source mapping levers, same bucket hardening and lifecycle, same Glue projection and Athena scan cap, same IAM and alarms. In a fresh environment the resulting infrastructure is byte-for-byte what ADR-0012 produced, differently packaged.
- The Glue database and Athena workgroup keep their `results` names because they name the dataset, not the module.
