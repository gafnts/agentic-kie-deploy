...
The entire pipeline is provisioned with Terraform, using a remote S3 backend with native state locking, organized as small per-concern modules (bucket, queue, table, registry, extractor, alarms; uploader planned).
