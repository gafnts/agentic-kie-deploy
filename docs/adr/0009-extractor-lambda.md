...
A second detail, narrower in scope but in the same "silent if missed" category, is what `@traceable` ships when things fail. The decorator captures the wrapped function's positional arguments and any raised exception verbatim. For `extract(bucket, key, document_id)` that means the ingestion bucket name (which reveals the `${project}-${env}-ingestion-${suffix}` naming convention and the random suffix that makes the bucket addressable) and any `botocore.exceptions.ClientError`, whose default message string embeds the calling account ID, the assumed role ARN, and the full resource ARN. None of those identifiers are prompts or completions—they are infrastructure facts about the AWS account—but the SaaS trail they leave to LangChain's API is the same. The boundary fix is small and lives at the `extract()` site: a `process_inputs` hook on the decorator drops `bucket` and `key` (CloudWatch already has both, and `document_id` alone is enough to correlate a trace back to a DynamoDB row), and a `ClientError` catch translates the boto3 error into a sanitized `ExtractionError("aws:<Code>")` with `from None` so the original `__cause__` is not chained into the trace. CloudWatch keeps the full `logger.exception` traceback, the DynamoDB `error.message` is still useful to an API consumer ("aws:AccessDenied" is the right granularity for a polling client), and only the code—not the resource it was raised against—leaves the account. Non-`ClientError` failures inside `agentic_kie` or the Gemini client propagate unchanged, because their messages are usually genuinely useful in a trace and do not carry account identifiers.
