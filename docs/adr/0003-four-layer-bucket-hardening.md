...
### Operational settings (non-hardening)

Three settings on the ingestion bucket sit outside the four-layer hardening posture but are wired into the same module so the bucket arrives operationally sound, not just locked down. They are recorded here for completeness rather than as separate decisions:

- **Versioning enabled** on the ingestion bucket—recovery substrate against accidental overwrite or delete. Non-current versions are expired by the lifecycle rule below so versioning does not become an unbounded storage cost.
- **Server-access logging** into a sibling `${bucket}-logs` bucket (PAB on, AES256 at rest, 90-day expiry on logs). Request-level audit trail of every operation on the ingestion bucket, independent of CloudTrail data events. The logs bucket carries the same Public Access Block and ownership posture as the ingestion bucket so the audit trail itself is not a new exposure.
